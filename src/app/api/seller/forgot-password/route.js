 ...
 * Handles password reset requests for sellers
 */
export async function POST(request) {
  try {
    ...
    // Security: Don't reveal if seller exists or not
    if (!seller) {
      return NextResponse.json({
    ...
    // Create reset link
    const resetLink = `${process.env.NEXT_PUBLIC_SITE_URL || "http://localhost:3000"}/seller/reset-password?token=${token}&email=${encodeURIComponent(emailNormalized)}`;
